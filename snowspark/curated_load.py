...
def main(session: snowpark.Session):
    ...
    df_india_targets = session.table(
        "SNOWPARK_DB.RAW.INDIA_SALES_TARGETS"
    ).withColumn(
        "TARGET_DATE",
        to_date(col("ORDER_MONTH"), "MON-YY")
    ).withColumn(
        "YEAR",
        year(col("TARGET_DATE"))
    ).withColumn(
        "MONTH",
        month(col("TARGET_DATE"))
    )
    
    df_india_actuals = df_sales_std \
        .filter(col("COUNTRY") == "INDIA") \
        .withColumn("YEAR", year(col("ORDER_DATE"))) \
        .withColumn("MONTH", month(col("ORDER_DATE"))) \
        .groupBy("YEAR", "MONTH", "CATEGORY") \
        .agg(
            sum_("SALES_AMOUNT").alias("ACTUAL_SALES")
        )
    
    df_india_vs_target = df_india_actuals.join(
        df_india_targets,
        ...
        (df_india_actuals["ACTUAL_SALES"] - df_india_targets["TARGET_AMOUNT"]).alias("VARIANCE"),
    ...
    df_top_products = df_product_sales.withColumn(
        "PRODUCT_RANK",
